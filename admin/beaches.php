...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($action == 'create' || $action == 'update') {
            ...
            if (empty($name)) {
                throw new Exception("Beach name is required.");
            }
            ...
            if ($action == 'create') {
                $stmt = $conn->prepare("INSERT INTO beaches (name, state, description, status, image) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$name, $state, $description, $status, $image_path]);
                $_SESSION['success'] = "Beach created successfully!";
            } else {
                $stmt = $conn->prepare("UPDATE beaches SET name=?, state=?, description=?, status=?, image=? WHERE id=?");
                $stmt->execute([$name, $state, $description, $status, $image_path, $id]);
...
if (!empty($search)) {
    $where[] = "(name LIKE :search OR state LIKE :search)";
